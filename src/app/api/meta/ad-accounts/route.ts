...
export async function GET(request: Request) {
  ...
  let query = supabase
    ...
    .select('id, ad_account_id, name, business_manager_id, account_status, currency, is_active')
  ...
  if (businessManagerId && businessManagerId !== 'all') {
    query = query.eq('business_manager_id', businessManagerId)
  }

  const { data: adAccounts, error } = await query

  if (error) {
    console.error('Ad accounts query error:', error)
    return NextResponse.json({ data: [] })
  }

  return NextResponse.json({ data: adAccounts || [] })
}
